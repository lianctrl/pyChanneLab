...
class ProtocolCreatorApp:
    ...
    def add_variable_sub_protocol(self):
        sub_type = simpledialog.askstring("Variable Sub-Protocol",
                                          "Enter Variable Type (Voltage or Concentration):")
        if sub_type is None or sub_type.lower() not in ["voltage", "concentration"]:
            return
        ...
        time = simpledialog.askfloat("Variable Sub-Protocol", "Enter Time:")
        if time is None:
            return

        values = [float(val.strip()) for val in values_str.split(",")]

        sub_protocol = SubProtocol(sub_type=sub_type.capitalize(), values=values, time=time)
        protocol = VoltageProtocol(name=self.name_var.get())
        protocol.add_sub_protocol(sub_protocol)
        self.protocols.append(protocol)
        self.update_protocol_table()
    ...
    def show_experiment_window(self):
        ...
        def create_experiment():
            if selected_protocols:
                ...
                # Plot voltage-time graphs for all the joined protocols
                plt.figure(figsize=(8, 6))
                for protocol in selected_protocols:
                    for sub_protocol in protocol.sub_protocols:
                        if sub_protocol.sub_type == "Holding":
                            # Plot constant voltage (or concentration) for the holding sub-protocol
                            voltage = sub_protocol.values[0]
                            time = sub_protocol.time
                            plt.plot([0, time], [voltage, voltage], label=f"{protocol.name} Holding")
                        elif sub_protocol.sub_type in ["Voltage", "Concentration"]:
                            # Plot variable voltage (or concentration) for the variable sub-protocol
                            for value in sub_protocol.values:
                                voltage = value
                                time = sub_protocol.time
                                plt.plot([0, time], [voltage, voltage], label=f"{protocol.name} {sub_protocol.sub_type}={value}")

                plt.xlabel("Time")
                plt.ylabel("Voltage (or Concentration)")
                plt.legend()
