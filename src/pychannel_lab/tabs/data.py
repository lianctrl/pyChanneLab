...
from core.curve_fitter import fit_curve, eval_curve, CURVE_FUNCTIONS, CURVE_LABELS, PROTOCOL_CURVE_DEFAULTS
    ...
    for pk in PROTOCOL_KEYS:
        with st.expander(PROTOCOL_LABELS[pk], expanded=(pk == "activation")):
            uploaded = st.file_uploader(
                f"CSV — {PROTOCOL_LABELS[pk]}", type=["csv"], key=f"upload_{pk}",
            )
            ...
            if pk in ss.exp_data:
                ...
                if (
                    ...
                    popt, perr, ok = fit_curve(x, y, cft, fix_baseline=fix_base, fix_amplitude=fix_amp)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    marker=dict(size=7, color="#1f77b4"), **err_kw,
                ))
