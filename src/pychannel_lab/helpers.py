...
def _network_figure(definition: MSMDefinition) -> go.Figure:
    ...
    for tr in definition.transitions:
        ...
        annotations.append(dict(
            ...
            showarrow=True, arrowhead=3, arrowsize=1.2, arrowwidth=1.5, arrowcolor="#888",
            text=label, font=dict(size=9, color="#555"),
            bgcolor="rgba(255,255,255,0.7)", hovertext=tr.rate_expr,
        ))
    ...
    for stype in ["closed", "inactivated", "open"]:
        ...
        fig.add_trace(go.Scatter(
            ...
            name=TYPE_LABELS[stype], text=names,
            textposition="middle center",
            textfont=dict(size=11, color="white", family="monospace"),
            marker=dict(size=42, color=TYPE_COLORS[stype], line=dict(width=2, color="white")),
            hovertext=[f"{n} ({stype})" for n in names], hoverinfo="text",
        ))
    ...
    fig.update_layout(
        ...
        height=420, margin=dict(t=20, b=60, l=20, r=20),
...
def _voltage_preview(proto_key: str) -> go.Figure:
    ...
    if proto_key == "activation":
        ...
        for V in [ss.act_cfg.v_min, (ss.act_cfg.v_min + ss.act_cfg.v_max) / 2, ss.act_cfg.v_max]:
            vf = p.get_voltage_function(V)
            fig.add_trace(go.Scatter(x=t, y=[vf(ti) for ti in t], mode="lines", name=f"V_test={V:.0f} mV"))

    elif proto_key == "inactivation":
        p = InactivationProtocol(ss.inact_cfg)
        for V in [ss.inact_cfg.v_min, (ss.inact_cfg.v_min + ss.inact_cfg.v_max) / 2, ss.inact_cfg.v_max]:
            vf = p.get_voltage_function(V)
            fig.add_trace(go.Scatter(x=t, y=[vf(ti) for ti in t], mode="lines", name=f"V_cond={V:.0f} mV"))
        ...
        for tp in [times[0], times[len(times) // 2], times[-1]]:
            ...
            fig.add_trace(go.Scatter(x=t, y=[vf(ti) for ti in t], mode="lines", name=f"t_pulse={tp:.3f} s"))
        ...
        for tp in [times[0], times[len(times) // 2], times[-1]]:
            vf = p.get_voltage_function(tp)
            fig.add_trace(go.Scatter(x=t, y=[vf(ti) for ti in t], mode="lines", name=f"t_rec={tp:.3f} s"))
    ...
    for (row, col), pk in zip(positions, PROTOCOL_KEYS):
        ...
        if exp is not None:
            ...
            err_kw = (
                ...
                if y_err is not None else {}
            )
            fig.add_trace(go.Scatter(
                x=x_exp, y=y_exp, mode="markers", name="Experimental",
                marker=dict(color="#1f77b4", size=8),
                showlegend=(row == 1 and col == 1), **err_kw,
            ), row=row, col=col)

        fig.add_trace(go.Scatter(
            x=x_sim, y=y_sim, mode="lines", name="Simulated",
...
def generate_run_script() -> str:
    ...
    w('# ── optimisation hyperparameters (as set in the GUI when script was exported) ─')
    ...
    w('    {"timestamp": "' + ts + '",')
    w('     "parameters": {p.name: float(v) for p, v in zip(msm_def.parameters, fitted_params)},')
