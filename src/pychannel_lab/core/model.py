...
class IonChannelModel:
    ...
    def equations(self, state, t: float, voltage_func):
        ...
        dC1 = 4*alpha*C0 + 2*beta*C2 + (k_IC/f**3)*I1 - (k_CI*f**3 + beta + 3*alpha)*C1
        dC2 = 3*alpha*C1 + 3*beta*C3 + (k_IC/f**2)*I2 - (k_CI*f**2 + 2*beta + 2*alpha)*C2
        dC3 = 2*alpha*C2 + 4*beta*C4 + (k_IC/f  )*I3 - (k_CI*f   + 3*beta +   alpha)*C3
        dC4 = alpha*C3 + k_OC*O + k_IC*I4 - (k_CI + k_CO + 4*beta)*C4

        # Inactivated states
        dI0 = beta*f*I1   + k_CI*f**4*C0 - (k_IC/f**4 + 4*(alpha/f))*I0
        dI1 = 4*(alpha/f)*I0 + 2*beta*f*I2 + k_CI*f**3*C1 - (k_IC/f**3 + beta*f + 3*(alpha/f))*I1
        dI2 = 3*(alpha/f)*I1 + 3*beta*f*I3 + k_CI*f**2*C2 - (k_IC/f**2 + 2*beta*f + 2*(alpha/f))*I2
        dI3 = 2*(alpha/f)*I2 + 4*beta*f*I4 + k_CI*f  *C3 - (k_IC/f   + 3*beta*f +   (alpha/f))*I3
